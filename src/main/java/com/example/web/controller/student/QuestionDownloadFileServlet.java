...
@WebServlet(name = "QuestionDownloadFileServlet", value = "/QuestionDownload")
public class QuestionDownloadFileServlet extends HttpServlet {
    @Override
    protected void doGet(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
        ...
        String dataDirectory = req.getServletContext().getRealPath("/question/"+questionid);
        File file = new File(dataDirectory);
        if(!file.exists()){
            System.out.println("找不到文件");
            resp.setContentType("text/html;charset=utf-8");
            resp.sendRedirect("StudentQuestionDetail?questionid="+questionid+"&questionUser="+questionUser+"&courseid="+courseid);
        }else {
